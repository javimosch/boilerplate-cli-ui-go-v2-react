...
const (
	pidFile = "/tmp/boilerplate-cli-ui-go-v2-react.pid"
	logFile = "/tmp/boilerplate-cli-ui-go-v2-react.log"
)
...
func daemonStart(host string, port int) {
	...
	if err := cmd.Start(); err != nil {
		die(ExitInternal, "spawn_failed",
			...
			fmt.Sprintf("boilerplate-cli-ui-go-v2-react serve --port %d", port))
	}
...
func daemonStop(port int) {
	...
	if err != nil {
		die(ExitExternal, "shutdown_failed",
			...
			fmt.Sprintf("boilerplate-cli-ui-go-v2-react daemon status --port %d", port))
	}
	resp.Body.Close()
