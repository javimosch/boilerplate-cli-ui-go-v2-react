// Command boilerplate-cli-ui-go-v2-react is a CLI with an embedded web UI.
//
...
func handleDaemon() {
	if len(os.Args) < 3 {
		die(ExitMissingArg, "missing_argument",
			...
			"boilerplate-cli-ui-go-v2-react daemon status")
	}
	sub := os.Args[2]
...
func handleVersion(args []string) {
	if hasFlag(args, "--json") {
		out, _ := json.Marshal(map[string]string{
			"version": Version,
			"name":    "boilerplate-cli-ui-go-v2-react",
		})
		fmt.Println(string(out))
		return
	}
	fmt.Printf("boilerplate-cli-ui-go-v2-react v%s\n", Version)
}
...
func printHelp() {
	e := os.Stderr
	fmt.Fprintln(e, "boilerplate-cli-ui-go-v2-react - Go CLI with an embedded React web UI")
	fmt.Fprintln(e)
	fmt.Fprintln(e, "Usage:")
	fmt.Fprintln(e, "  boilerplate-cli-ui-go-v2-react <command> [options]")
	fmt.Fprintln(e)
